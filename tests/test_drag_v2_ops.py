...
def load_modules():
    ...
    result = {}
    for name in ("image_conveyor_server", "image_conveyor_library_ops", "image_conveyor_drag_ops", "image_conveyor_drag_v2_ops"):
...
class DragV2OperationsTest(unittest.TestCase):
    def setUp(self):
        ...
        self.input.mkdir(); self.cache.mkdir()
        self.service = server.InputLibrary(
            str(self.input), str(self.cache), snapshot_ttl=60.0,
    ...
    def character(self, preset):
        ...
        character = next(x for x in registry.ensure_for_presets(self.service.preset_store.list()) if x["preset_id"] == preset["id"])
    ...
    def test_identical_destination_is_collapsed_without_suffix(self):
        ...
        saved = next(x for x in self.service.preset_store.list() if x["id"] == preset["id"])
    ...
    def test_character_materialization_moves_and_relinks_refs(self):
        ...
        refreshed = next(x for x in registry.ensure_for_presets(self.service.preset_store.list()) if x["preset_id"] == preset["id"])
    ...
    def test_retroactive_migration_collapses_orphan_copy_era_source(self):
        ...
        self.assertTrue(any(x["relative_path"] == "legacy/a.png" and x["keep_path"] == canonical for x in result["moved"]))

    def test_protected_character_source_is_not_moved_or_catalogued(self):
        source = self.image("source/a.png")
        preset = self.service.preset_store.create("Mara", slots())
        registry, character = self.character(preset)
        result = drag.materialize_character_files(self.service, preset["id"], ["source/a.png"], ["source/a.png"])
